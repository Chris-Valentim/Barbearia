...
const Summary = () => {
  ...
  const { date, professional, services, totalPrice, totalDuration, schedule } = useScheduling()
  ...
  function renderService(name: string, i: number) {
    return (
      <div key={i} className="flex items-center bg-zinc-700 rounded-md">
        ...
        <span className="text-sm font-light text-zinc-300 px-2">
          {name}
        </span>
      </div>
    )
  ...
  return (
    <div className="flex flex-col bg-zinc-950 rounded-lg w-96 lg:w-80">
      ...
      <div className="flex flex-col p-5 gap-6 border-b border-zinc-800">
        ...
        <div className="flex flex-col gap-3">
          ...
          <span className="flex gap-2 flex-wrap text-sm text-white">
            {services.length ? services.map((service, i) => renderService(service.name, i + 1)) : 'Não selecionado'}
          </span>
        </div>
        <div className="flex flex-col gap-3">
          <span className="text-xs uppercase text-zinc-400">
        ...
        <div className="flex flex-col gap-3">
          ...
          <span className="text-sm text-white">
            ...
            {date && `às ${String(date?.getHours()).padStart(2, '0')}:${String(date?.getMinutes()).padStart(2, '0')}h`}
          </span>
        </div>
      </div>
      <div className="flex justify-between items-center border-b border-zinc-800 p-5">
        <span className="text uppercase text-zinc-400">
          Valor Total
        </span>
      ...
      <div className="p-5">
        <button
          className={`flex justify-center items-center text-sm font-semibold ${canFinish() ? 'bg-yellow-400' : 'bg-zinc-600'} text-zinc-900 w-full py-3 rounded-lg`}
          disabled={!canFinish()}
